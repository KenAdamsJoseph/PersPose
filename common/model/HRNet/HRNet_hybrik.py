...
class PoseHighResolutionNet(nn.Module):
    ...
    def forward(self, x, cam_info=None):
        # with torch.no_grad():
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        if self.generate_hm:
            ...
            if self.generate_feat:
                # Classification Head
                y = self.incre_modules[0](y_list[0])  # 一个bottleneck 三层
                for i in range(len(self.downsamp_modules)):  # 三个bottleneck
                    y = self.incre_modules[i + 1](y_list[i + 1]) + \
                        self.downsamp_modules[i](y)

                y = self.final_feat_layer(y)  # bs,1024,8,4 -> bs,2048,8,4
                ...
                return y_list[0], feat  # out_heatmap, feat
            else:
                return 0  # out_heatmap
        else:
...
def get_hrnet(type_name, num_joints, depth_dim, init_weights=True, pretrained='', **kwargs):
    # https://github.com/HRNet/HRNet-Human-Pose-Estimation/blob/master/README.md
    cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'w48_256x192_adam_lr1e-3.yaml')
