...
def swing_radian(pred_twist, gt_twist, reduce='mean'):  # swing angular error
    gt_twist = F.normalize(gt_twist, p=2, dim=-1)
    pred_twist = F.normalize(pred_twist, p=2, dim=-1)
    cos_theta = (gt_twist * pred_twist).sum(dim=-1)
    theta_rad = torch.acos(torch.clip(cos_theta, -1, 1))  # avoid 1+
    if reduce == 'mean':
...
def j3d_loss(predicted_3d_pos, inputs_3d):
    w_mpjpe = torch.tensor([  # 1, 1, 2.5, 2.5, 1, 2.5, 2.5, 1, 1, 1, 1.5, 1.5, 4, 4, 1.5, 4, 4 # h36m.j17
        1, 1, 1, 1, 2.5, 2.5, 1, 2.5, 2.5, 1, 2.5, 2.5, 1, 1, 1, 1.5, 1.5, 1.5, 4, 4, 4, 4, 4, 4,  # smpl.j24
        # 1, 1, 2.5, 2.5, 2.5, 2.5, 1, 2.5, 2.5, 2.5, 2.5, 1, 1, 1, 1.5, 1.5, 4, 4, 1, 4, 1.5, 4, 4, 1, 4  # h36m.j25
    ]).cuda()
